...
(defn ^:export init []
  (let [surface (surface)
        ...
        timer3 (goog.Timer. (/ 1000 10))
        stateAtom (atom (init-game-state surface))]

    (events/listen timer2  goog.Timer/TICK #(update-canvas @stateAtom surface))
    (events/listen timer1  goog.Timer/TICK #(do
                                                  (playerMoveWrapping stateAtom)
                                                  ;(update-canvas @stateAtom surface)
                                                  ))
    (events/listen timer3  goog.Timer/TICK #(do
                                                  (autoMove stateAtom)
                                                  ;(update-canvas @stateAtom surface)
                                                  ))
